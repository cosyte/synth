...
attw` and `find dist -name '*.d.*ts' -delete && pnpm attw` printed the sentence and exited 0.
  ...
  **The scope is narrower than it first looks.** With the root entry intact and one subpath's
  ...
  `rm -rf dist`, `clean` and the build window all produce.
  ...
  Because the post-check reads printed output, the options that would hide it are refused rather than
  tolerated: `--quiet`, `-q`, `--format json`, `--format=json`, and a `.attw.json` setting `quiet` or
  `format` were each measured to make the sentence unreadable while `attw` still exited 0.
  `--config-path` is refused by inference. The refusal is by option name, wholesale, not by value.

  `--profile node16` is unchanged and still applied, forwarded through the wrapper rather than baked
  into it, with the forwarding pinned by a test. **Nothing about the published package changes** — no
  runtime code, no types, no generated byte, an identical tarball. This changes what the release gate
  will let through. `test/scripts/attw-gate.test.ts` pins the upstream exit-0 as well as the fix, so
