...
attw` and `find dist -name '*.d.*ts' -delete && pnpm attw` printed the sentence and exited 0.
  ...
  **The scope is narrower than it first looks.** With the root entry intact and one subpath's
  ...
  those it is looking at: it claims the exit-0 counterfactual only when no declared declaration
  survives, and otherwise says `attw` would have reported an untyped resolution and exited 1, with
  the artifact name being what the preflight adds. A first version keyed that on _any_ missing
  declaration and was false on this package's own eight-entry-point layout.
  ...
  **allow-list** — `--profile` and `--no-definitely-typed`, nothing else. Six routes were measured to
  ...
  types, not one generated byte under `dist/`. The tarball is not byte-identical, because
  `CHANGELOG.md` is in `files` and this entry is in it; nothing a consumer imports or executes moves.
