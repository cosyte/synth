...
the strongest one. It claims the exit-0 counterfactual only when no declared declaration survives;
when some survive it says `attw` would have reported an untyped resolution and exited 1, and that
what the preflight adds there is the name of the artifact to rebuild. A first version keyed that on
*any* missing declaration and was false on this package's own eight-entry-point layout — a gate that
reds correctly and then explains itself with a falsehood teaches the wider, wrong story.
...
tarball is not byte-identical, because `CHANGELOG.md` is in `files` and this change adds an entry to
it — but nothing a consumer imports or executes moves. This is a change to what the release gate will
let through.
